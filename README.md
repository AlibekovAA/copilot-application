...
- **[Нагрузочное тестирование Python бекенда](backend/llm_service/perf/README.md)** - инструменты и документация для тестирования производительности
...
**Нагрузка** (200 пользователей, 300 сек):

- **RPS**: 826 запросов/сек
- **Успешность**: 100%
- **Среднее время отклика**: 0.23 сек
- **P95**: 0.99 сек, **P99**: 1.58 сек
...
**Вывод**: Go бекенд стабильно обрабатывает до 800+ RPS без ошибок.
...
- **RPS**: 166 запросов/сек
- **Успешность**: 100%
- **Среднее время отклика**: 1.2 сек
- **P95**: 4.04 сек, **P99**: 6.58 сек

**Тестируемые эндпоинты**:

- `POST /conversations` - создание новой беседы
- `POST /chat` - отправка сообщения в чат
- `GET /conversations` - получение списка бесед
- `GET /conversations/{id}/messages` - получение сообщений беседы
- `GET /health` - проверка здоровья сервиса

**Вывод**: LLM сервис стабильно обрабатывает нагрузку. Время отклика зависит от сложности обработки сообщений и работы с LLM API.
